...
const CoinCard = React.memo(({ coin }) => {
    ...
    const styles = {
        ...
        btn: { padding: '6px 10px', borderRadius: 6, color: 'white', cursor: 'pointer', backgroundColor: '#000', border: hover ? '.5px solid #ffffff57' : '1px solid #000' }
    }
    ...
    return (
        <div style={styles.card}>
            ...
            <button onClick={() => add(coin)} style={styles.btn} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>Add to Portfolio</button>
        </div>
    );
});
